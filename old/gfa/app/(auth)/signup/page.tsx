...
export default function SignupPage() {
  ...
  async function handleSignup() {
    ...
    try {
      ...
      setInfo(
        'Account created. If your project requires email confirmation, check your inbox and click the link—then sign in. You can disable “Confirm email” under Supabase → Authentication → Providers → Email for local demos.'
      )
      setLoading(false)
    } catch (e) {
      setError(
    ...
    <div className="relative flex min-h-screen items-center justify-center bg-gfa-canvas px-4">
      ...
      <div className="relative w-full max-w-[400px] space-y-10">
        ...
        <Card className="bg-gfa-rose/30">
          <CardHeader>
            <CardTitle className="font-display text-2xl font-normal">Create account</CardTitle>
            <CardDescription>Join in a minute.</CardDescription>
          </CardHeader>
          <CardContent className="pt-0">
            ...
            {info ? (
              <div className="mb-5 rounded-[10px] border border-gfa-accent/25 bg-gfa-accent-muted px-4 py-3 text-[13px] leading-relaxed text-gfa-fg/90">
                {info}
                <Link href="/login" className="mt-3 block text-gfa-accent hover:text-gfa-accent-bright">
                  Go to sign in →
                </Link>
              </div>
            ) : null}
            ...
            <Button type="button" onClick={handleSignup} disabled={loading} size="pill" className="mt-6 w-full">
              {loading ? (
                <>
                  <Spinner />
                  Creating…
          ...
          <p className="mt-8 text-center text-[13px] text-gfa-muted">
            Already registered?{' '}
            <Link href="/login" className="text-gfa-accent transition-colors hover:text-gfa-accent-bright">
              Sign in
            </Link>
          </p>
          </CardContent>
