  ...
  cookies: {
    ...
    setAll(cookiesToSet) {
      ...
      cookiesToSet.forEach(({ name, value, options }) => {
        // Supabase's cookie options type doesn't perfectly match `document.cookie` serialization.
        // We only read the fields we support in `serializeCookie`.
        document.cookie = serializeCookie(
          name,
          value,
          options as Parameters<typeof serializeCookie>[2]
        )
      })
    },
  },
